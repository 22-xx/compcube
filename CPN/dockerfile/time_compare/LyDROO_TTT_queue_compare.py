...
def MSL_DRL(n,TTT):
    ...
    Reuest_time = 10
    SLA_time = 0.2 # time comsumption threshold in J per time slot
    ...
    price_cost = np.zeros((n, D, N, M))
    cost = np.zeros(n) # objective values after solving problem (26)
    ...
    for i_idx in range(n):
        ...
        #100000,Q值可能后期为0
        h_new=VM_read()
        workload[i_idx,:]=work_read(i_idx,N)**0.18
        ...
        if (i_idx==0 or (i_idx+TTT)%TTT == 0):
            ...
            for m in m_list:
              ...
              r_list.append(Algo1_NUM(decision_new[i_idx,:,:],price[i_idx,:,:],dis,m,workload[i_idx,:],Q[i_idx,:,:],H[i_idx,:,:],Y[i_idx,:,:],TTT, V))
              # 取出最优值f
            ...
            for n in range(N):
                for d in range(D):
                    # 扩展价格维数
                    price_cost[i_idx,d, n, :] = price[i_idx,d, :]
            # print('m_list[k_idx_his[-1]]m_list[k_idx_his[-1]]',np.shape(m_list[k_idx_his[-1]]))
            # print('type(r_list[k_idx_his[-1]])',r_list[k_idx_his[-1]])
            cost[i_idx:i_idx+TTT-1] = sum(sum(sum(m_list[k_idx_his[-1]].reshape(D, N, M) * price_cost[i_idx,d, n, :])))
